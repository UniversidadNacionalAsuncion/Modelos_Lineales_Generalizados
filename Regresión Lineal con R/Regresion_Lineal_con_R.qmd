...
```{r}
with(cars, plot(speed, dist, pch = 19, col = "darkred",
...
Este valor indica que existe una **fuerte relación lineal positiva**
entre la velocidad y la distancia de frenado. El valor del coeficiente de correlación de Pearson es `r round(coef_correlacion, 2)`
...
```{r}
regresion <- lm(dist ~ speed, data = cars)
regresion
```
...
-   **Coeficiente de `speed`**: Por cada aumento de **una unidad en la velocidad** (1 mph), la **distancia de frenado aumenta en
...
El **coeficiente de determinación** ($R^2 = 0.6511$) indica que el
modelo explica aproximadamente **65% de la variabilidad** en la
distancia de frenado a partir de la velocidad.
...
library(car)
crPlots(regresion)
mean(residuals(regresion))  # La media de los residuos debería ser cercana a 0
```

Observamos que **no hay una desviación significativa de la linealidad**.
...
**(3) Supuesto de no autocorrelación**
