...
Para probar las hipótesis utilizamos la estadística F de Fisher-Snedecor en un Análisis de la Varianza (ANOVA). Si se rechaza $H_0$ se concluye que existe al menos una variable que aporta de manera significativa para modelar la respuesta $Y$.

| Fuente | gl | Suma de <br> Cuadrados | Cuadrado Medio | F | P-valor |
| :---: | :---: | :--- | :---: | :---: | :---: |
| Regresión | 1 | SCR | CMR $=$ SCR $/ 1$ | CMR $/$ CME |  |
...
-   **millas**: Millas recorridas por galón de combustible (mpg), que será nuestra
    **variable respuesta**.
...
rstatix::get_summary_stats(mtautos)
write.table(rstatix::get_summary_stats(mtautos), file = "Resumen_mtautos.txt",
            sep = "\t")
```
...
Calculamos los p valores asociados a los coeficientes de correlación. Se plantean las siguientes hipótesis.

$H_0: \rho=0$ (No existe relación lineal entre las dos variables analizadas)

$H_1: \rho \neq 0$ (Existe relación lineal entre las dos variables analizadas)

```{r}
matriz_correlaciones <- rstatix::cor_test(with(mtautos, data.frame(cbind(cilindrada, peso, millas))))
